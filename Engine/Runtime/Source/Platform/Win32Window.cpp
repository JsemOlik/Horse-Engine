...
Window::Window(const WindowDesc &desc)
  ...
  RECT windowRect = {0, 0, static_cast<LONG>(desc.Width),
                     static_cast<LONG>(desc.Height)};
  AdjustWindowRect(&windowRect, style, FALSE);

  m_Hwnd = CreateWindowExW(0, WINDOW_CLASS_NAME, desc.Title.c_str(), style,
...
LRESULT CALLBACK Window::WindowProc(HWND hwnd, UINT msg, WPARAM wParam,
  ...
  case WM_KEYDOWN:
  case WM_SYSKEYDOWN:
    Input::UpdateKeyState(static_cast<int>(wParam), true);
    return 0;

  case WM_KEYUP:
  case WM_SYSKEYUP:
    Input::UpdateKeyState(static_cast<int>(wParam), false);
    return 0;

  case WM_LBUTTONDOWN:
    Input::UpdateMouseButtonState(0, true);
    return 0;
  case WM_LBUTTONUP:
    Input::UpdateMouseButtonState(0, false);
    return 0;
  case WM_RBUTTONDOWN:
    Input::UpdateMouseButtonState(1, true);
    return 0;
  case WM_RBUTTONUP:
    Input::UpdateMouseButtonState(1, false);
    return 0;
  case WM_MBUTTONDOWN:
    Input::UpdateMouseButtonState(2, true);
    return 0;
  case WM_MBUTTONUP:
    Input::UpdateMouseButtonState(2, false);
    return 0;
